...
namespace TENTAC_HRM.Forms.Category
{
    ...
    {
        string _IDLuong, _ThueTNCN, _GhiChu;
        int active_value, _DongBHXH, _DongBHYT, _DongBHTN, _MienThue, _ThueCoDinh, _CongDoan;
        ...
        {
            string sql = string.Format("select Id, TuNgay, DenNgay,FORMAT(MucLuong,'N2') as MucLuong, IsActive, PT_ThueTNCN from tbl_NhanVienLuong where MaNhanVien = '{0}'", cbo_NhanVien.SelectedValue.ToString());
        ...
        private void save_data()
        {
            try
            {
                ...
                Values ({SQLHelper.rpStr(_ma_nhan_vien)}, {SQLHelper.rpI(active_value)}, {SQLHelper.rpDT(_TuNgay)}, {SQLHelper.rpDT(_DenNgay)},
        ...
        private void dgv_luong_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                edit = true;
                _IDLuong = dgv_Luong.CurrentRow.Cells["ID"].Value.ToString();
                string sql = string.Format("select * from tbl_NhanVienLuong where ID = {0}", _IDLuong);
                DataTable dataTable = new DataTable();
                dataTable = SQLHelper.ExecuteDt(sql);
                if (dataTable.Rows.Count > 0)
                {
                    txt_MucLuong.Text = decimal.Parse(dataTable.Rows[0]["MucLuong"].ToString()).ToString("N2", CultureInfo.InvariantCulture);
