using System;

namespace TENTAC_HRM.Models.BaoBieuModel
{
    internal class TinhCongModel
    {
        private int _ID;

        private string _MaNhanVien;

        private string _TenNhanVien;

        private int _MaChamCong;

        private DateTime _Ngay;

        private string _Thu;

        private string _Ca;

        private string _GioVao;

        private string _GioRa;

        private string _Cong;

        private string _Gio;

        private int _Tre;

        private int _VeSom;

        private int _VeTre;

        private string _TC1;

        private string _TC2;

        private string _TC3;

        private string _TC4;

        private string _TongGio;

        private string _DemCong;

        private string _KyHieu;

        private string _KyHieuPhu;

        private string _PhongBan;

        private string _VaoSom;

        public int ID
        {
            get
            {
                return this._ID;
            }
            set
            {
                this._ID = value;
            }
        }

        public string MaNhanVien
        {
            get
            {
                return this._MaNhanVien;
            }
            set
            {
                this._MaNhanVien = value;
            }
        }

        public string TenNhanVien
        {
            get
            {
                return this._TenNhanVien;
            }
            set
            {
                this._TenNhanVien = value;
            }
        }

        public int MaChamCong
        {
            get
            {
                return this._MaChamCong;
            }
            set
            {
                this._MaChamCong = value;
            }
        }

        public DateTime Ngay
        {
            get
            {
                return this._Ngay;
            }
            set
            {
                this._Ngay = value;
            }
        }

        public string Thu
        {
            get
            {
                return this._Thu;
            }
            set
            {
                this._Thu = value;
            }
        }

        public string Ca
        {
            get
            {
                return this._Ca;
            }
            set
            {
                this._Ca = value;
            }
        }

        public string GioVao
        {
            get
            {
                return this._GioVao;
            }
            set
            {
                this._GioVao = value;
            }
        }

        public string GioRa
        {
            get
            {
                return this._GioRa;
            }
            set
            {
                this._GioRa = value;
            }
        }

        public string Cong
        {
            get
            {
                return this._Cong;
            }
            set
            {
                this._Cong = value;
            }
        }

        public string Gio
        {
            get
            {
                return this._Gio;
            }
            set
            {
                this._Gio = value;
            }
        }

        public int Tre
        {
            get
            {
                return this._Tre;
            }
            set
            {
                this._Tre = value;
            }
        }

        public int VeSom
        {
            get
            {
                return this._VeSom;
            }
            set
            {
                this._VeSom = value;
            }
        }

        public int VeTre
        {
            get
            {
                return this._VeTre;
            }
            set
            {
                this._VeTre = value;
            }
        }

        public string TC1
        {
            get
            {
                return this._TC1;
            }
            set
            {
                this._TC1 = value;
            }
        }

        public string TC2
        {
            get
            {
                return this._TC2;
            }
            set
            {
                this._TC2 = value;
            }
        }

        public string TC3
        {
            get
            {
                return this._TC3;
            }
            set
            {
                this._TC3 = value;
            }
        }

        public string TC4
        {
            get
            {
                return this._TC4;
            }
            set
            {
                this._TC4 = value;
            }
        }

        public string TongGio
        {
            get
            {
                return this._TongGio;
            }
            set
            {
                this._TongGio = value;
            }
        }

        public string DemCong
        {
            get
            {
                return this._DemCong;
            }
            set
            {
                this._DemCong = value;
            }
        }

        public string KyHieu
        {
            get
            {
                return this._KyHieu;
            }
            set
            {
                this._KyHieu = value;
            }
        }

        public string KyHieuPhu
        {
            get
            {
                return this._KyHieuPhu;
            }
            set
            {
                this._KyHieuPhu = value;
            }
        }

        public string PhongBan
        {
            get
            {
                return this._PhongBan;
            }
            set
            {
                this._PhongBan = value;
            }
        }

        public string VaoSom
        {
            get
            {
                return this._VaoSom;
            }
            set
            {
                this._VaoSom = value;
            }
        }
    }
}
