using System;

namespace TENTAC_HRM.Models.QuanLyNhanVienModel
{
    internal class NhanVienDTO
    {
        private string _MaNhanVien;

        private string _TenNhanVien;

        private string _MaChamCong;

        private string _TenChamCong;

        private string _MaThe;

        private bool _GioiTinh;

        private DateTime _NgayVaoLamViec;

        private string _ChucVu;

        private DateTime _NgaySinh;

        private string _UserPassWord;

        private int _PhanQuyen;

        private string _UserEnable;

        private string _NoiSinh;

        private string _LoaiNhanVien;

        private int _CMND;

        private string _DienThoaiLienHe;

        private string _Email;

        private float _NgayPhep;

        private bool _TinhTrang;

        private byte[] _HinhAnh;

        private string _MaCongTy;

        private string _MaKhuVuc;

        private string _MaPhongBan;

        private string _MaChucVu;

        private string _TienLuong;

        private string _LuongHopDong;

        private string _DanToc;

        private string _QuocTich;

        private string _Skype;

        private string _Yahoo;

        private string _Facebook;

        private string _PassWord;

        private bool _DangThamGiaBaoHiem;

        private bool _NghiViecTamThoi;

        private bool _TinhLuongTheo;

        private bool _SanPhamOrCongDoan;

        private bool _NhanVienMoi;

        private DateTime _NgayCap;

        private string _NoiCap;

        private string _TrinhDo;

        private string _GhiChu;

        private bool _DaThanhToanGioiThieu;

        private DateTime _NgayKyHopDong;

        private float _ThoiHanHopDong;

        public string MaNhanVien
        {
            get
            {
                return _MaNhanVien;
            }
            set
            {
                _MaNhanVien = value;
            }
        }

        public string TenNhanVien
        {
            get
            {
                return _TenNhanVien;
            }
            set
            {
                _TenNhanVien = value;
            }
        }

        public string MaChamCong
        {
            get
            {
                return _MaChamCong;
            }
            set
            {
                _MaChamCong = value;
            }
        }

        public string TenChamCong
        {
            get
            {
                return _TenChamCong;
            }
            set
            {
                _TenChamCong = value;
            }
        }

        public string MaThe
        {
            get
            {
                return _MaThe;
            }
            set
            {
                _MaThe = value;
            }
        }

        public bool GioiTinh
        {
            get
            {
                return _GioiTinh;
            }
            set
            {
                _GioiTinh = value;
            }
        }

        public DateTime NgayVaoLamViec
        {
            get
            {
                return _NgayVaoLamViec;
            }
            set
            {
                _NgayVaoLamViec = value;
            }
        }

        public string ChucVu
        {
            get
            {
                return _ChucVu;
            }
            set
            {
                _ChucVu = value;
            }
        }

        public DateTime NgaySinh
        {
            get
            {
                return _NgaySinh;
            }
            set
            {
                _NgaySinh = value;
            }
        }

        public string UserPassWord
        {
            get
            {
                return _UserPassWord;
            }
            set
            {
                _UserPassWord = value;
            }
        }

        public int PhanQuyen
        {
            get
            {
                return _PhanQuyen;
            }
            set
            {
                _PhanQuyen = value;
            }
        }

        public string UserEnable
        {
            get
            {
                return _UserEnable;
            }
            set
            {
                _UserEnable = value;
            }
        }

        public string NoiSinh
        {
            get
            {
                return _NoiSinh;
            }
            set
            {
                _NoiSinh = value;
            }
        }

        public string LoaiNhanVien
        {
            get
            {
                return _LoaiNhanVien;
            }
            set
            {
                _LoaiNhanVien = value;
            }
        }

        public int CMND
        {
            get
            {
                return _CMND;
            }
            set
            {
                _CMND = value;
            }
        }

        public string DienThoaiLienHe
        {
            get
            {
                return _DienThoaiLienHe;
            }
            set
            {
                _DienThoaiLienHe = value;
            }
        }

        public string Email
        {
            get
            {
                return _Email;
            }
            set
            {
                _Email = value;
            }
        }

        public float NgayPhep
        {
            get
            {
                return _NgayPhep;
            }
            set
            {
                _NgayPhep = value;
            }
        }

        public bool TinhTrang
        {
            get
            {
                return _TinhTrang;
            }
            set
            {
                _TinhTrang = value;
            }
        }

        public byte[] HinhAnh
        {
            get
            {
                return _HinhAnh;
            }
            set
            {
                _HinhAnh = value;
            }
        }

        public string MaCongTy
        {
            get
            {
                return _MaCongTy;
            }
            set
            {
                _MaCongTy = value;
            }
        }

        public string MaKhuVuc
        {
            get
            {
                return _MaKhuVuc;
            }
            set
            {
                _MaKhuVuc = value;
            }
        }

        public string MaPhongBan
        {
            get
            {
                return _MaPhongBan;
            }
            set
            {
                _MaPhongBan = value;
            }
        }

        public string MaChucVu
        {
            get
            {
                return _MaChucVu;
            }
            set
            {
                _MaChucVu = value;
            }
        }

        public string TienLuong
        {
            get
            {
                return _TienLuong;
            }
            set
            {
                _TienLuong = value;
            }
        }

        public string LuongHopDong
        {
            get
            {
                return _LuongHopDong;
            }
            set
            {
                _LuongHopDong = value;
            }
        }

        public string DanToc
        {
            get
            {
                return _DanToc;
            }
            set
            {
                _DanToc = value;
            }
        }

        public string QuocTich
        {
            get
            {
                return _QuocTich;
            }
            set
            {
                _QuocTich = value;
            }
        }

        public string Skype
        {
            get
            {
                return _Skype;
            }
            set
            {
                _Skype = value;
            }
        }

        public string Yahoo
        {
            get
            {
                return _Yahoo;
            }
            set
            {
                _Yahoo = value;
            }
        }

        public string Facebook
        {
            get
            {
                return _Facebook;
            }
            set
            {
                _Facebook = value;
            }
        }

        public string PassWord
        {
            get
            {
                return _PassWord;
            }
            set
            {
                _PassWord = value;
            }
        }

        public bool DangThamGiaBaoHiem
        {
            get
            {
                return _DangThamGiaBaoHiem;
            }
            set
            {
                _DangThamGiaBaoHiem = value;
            }
        }

        public bool NghiViecTamThoi
        {
            get
            {
                return _NghiViecTamThoi;
            }
            set
            {
                _NghiViecTamThoi = value;
            }
        }

        public bool TinhLuongTheo
        {
            get
            {
                return _TinhLuongTheo;
            }
            set
            {
                _TinhLuongTheo = value;
            }
        }

        public bool SanPhamOrCongDoan
        {
            get
            {
                return _SanPhamOrCongDoan;
            }
            set
            {
                _SanPhamOrCongDoan = value;
            }
        }

        public bool NhanVienMoi
        {
            get
            {
                return _NhanVienMoi;
            }
            set
            {
                _NhanVienMoi = value;
            }
        }

        public DateTime NgayCap
        {
            get
            {
                return _NgayCap;
            }
            set
            {
                _NgayCap = value;
            }
        }

        public string NoiCap
        {
            get
            {
                return _NoiCap;
            }
            set
            {
                _NoiCap = value;
            }
        }

        public string TrinhDo
        {
            get
            {
                return _TrinhDo;
            }
            set
            {
                _TrinhDo = value;
            }
        }

        public string GhiChu
        {
            get
            {
                return _GhiChu;
            }
            set
            {
                _GhiChu = value;
            }
        }

        public bool DaThanhToanGioiThieu
        {
            get
            {
                return _DaThanhToanGioiThieu;
            }
            set
            {
                _DaThanhToanGioiThieu = value;
            }
        }

        public DateTime NgayKyHopDong
        {
            get
            {
                return _NgayKyHopDong;
            }
            set
            {
                _NgayKyHopDong = value;
            }
        }

        public float ThoiHanHopDong
        {
            get
            {
                return _ThoiHanHopDong;
            }
            set
            {
                _ThoiHanHopDong = value;
            }
        }
    }
}
